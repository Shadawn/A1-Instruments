...
Функция СемВер_Разобрать(Версия)
	...
	Если НЕ А1Э_Строки.ТолькоЦифры(Семвер.Основная) Тогда Возврат Ложь; КонецЕсли;
	Если НЕ А1Э_Строки.ТолькоЦифры(Семвер.Малая) Тогда Возврат Ложь; КонецЕсли;
	ОстатокВерсии = Сред(Версия, ВтораяТочка + 1);
	ДлинаОстатка = СтрДлина(ОстатокВерсии);
	Сч = 1;
	Пока ДлинаОстатка >= Сч И А1Э_Строки.ТолькоЦифры(Сред(ОстатокВерсии, Сч, 1)) Цикл
		Сч = Сч + 1;
	КонецЦикла;
