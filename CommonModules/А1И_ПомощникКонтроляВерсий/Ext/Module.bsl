...
#Если НЕ Клиент Тогда
	...
	Функция ФормаПриСозданииОбъектов(Форма, Отказ, МассивОписаний) Экспорт
		...
		А1И.А1БК_СохраняемоеПоле().ДобавитьОписание(МассивОписаний, "ВерсияПлатформы", "Строка", "Версия платформы", "СтраницаНастройки");
...
#Если Клиент Тогда
		...
		Попытка
			Для Каждого Строка Из Форма.Расширения Цикл
				...
				Попытка
					...
					УдалитьФайлы(ПутьСохраненияФайлаКонфигурации(Строка));	
				Исключение
					Ошибка = Истина;
	...
	Функция КонтекстВыполненияДляПустойБазы(Форма, ПутьКПустойБазе)
		Если ЗначениеЗаполнено(Форма.ВерсияПлатформы) Тогда
			СисИнфо = Новый СистемнаяИнформация;
		    КаталогПрограммы = СтрЗаменить(КаталогПрограммы(), СисИнфо.ВерсияПриложения, Форма.ВерсияПлатформы)	
		КонецЕсли;
		
		Возврат А1Э_Конфигуратор.НовыйКонтекстВыполнения(КаталогПрограммы, "File=""" + ПутьКПустойБазе + """", "", "", А1Э_Структуры.Создать(
