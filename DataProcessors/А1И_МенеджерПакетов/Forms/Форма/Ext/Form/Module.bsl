...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Пакет Из СписокПакетов Цикл
		...
		Строка.ИспользоватьОсновныеРолиДляВсехПользователей = А1Э_Общее_ЗначениеСвойства(Пакет, "useDefaultRolesForAllUsers");
	КонецЦикла;
...
Процедура УстановитьОтмеченные(Команда)
	КПроверкеЗависимостей = Новый Соответствие; 
	...
	Для Каждого Строка Из Пакеты Цикл
		...
		КПроверкеЗависимостей.Вставить(Строка.Имя, Строка);
		УстанавливаемыеИнтерактивно.Вставить(Строка.Имя, Строка);
	КонецЦикла;
	
	ПовторятьОбход = Истина;
	Пока ПовторятьОбход Цикл
		ПовторятьОбход = Ложь;
		Проверенные = Новый Массив;
		Для Каждого Пара Из КПроверкеЗависимостей Цикл
			Для Каждого ПараЗависимости Из Пара.Значение.Зависимости Цикл
				...
				Если ОбщиеЗависимости[ИмяЗависимости] <> Неопределено Тогда Продолжить; КонецЕсли;
				
				СтрокаЗависимости = Пакеты.НайтиСтроки(Новый Структура("Имя", ИмяЗависимости))[0];
				КПроверкеЗависимостей.Вставить(ИмяЗависимости, СтрокаЗависимости);
				ОбщиеЗависимости.Вставить(ИмяЗависимости, СтрокаЗависимости);
				ПовторятьОбход = Истина;
			КонецЦикла;
			Проверенные.Добавить(Пара.Ключ);
		КонецЦикла;
		Для Каждого ИмяЗависимости Из Проверенные Цикл
			КПроверкеЗависимостей.Удалить(ИмяЗависимости);
		КонецЦикла;
	КонецЦикла;
	УстанавливаемыеПоЗависимостям = Новый Массив;
	ВсеУстанавливаемые = Новый Массив;
	Для Каждого Пара Из ОбщиеЗависимости Цикл
		Строка = Пара.Значение;
		Если Строка.ДоступнаяБольшеТекущей Или Строка.ВерсияТекущая = "" Тогда
	...
	Иначе
		ТекстВопроса =  "Будут установлены/обновлены следующие расширения, от которых зависят выбранные:" + Символы.ПС
...
Процедура УстановитьОтмеченныеНаСервере(ВсеУстанавливаемые)
	...
	Для Каждого Имя Из ВсеУстанавливаемые Цикл
		...
		Расширение.ИспользоватьОсновныеРолиДляВсехПользователей = Строка.ИспользоватьОсновныеРолиДляВсехПользователей;
...
Процедура УстановитьМеханизмы(Команда)
	Модуль__А1Э_Механизмы = Вычислить("А1Э_Механизмы");
	Модуль__А1Э_Механизмы.УстановитьРасширение(, Пароль);
КонецПроцедуры
