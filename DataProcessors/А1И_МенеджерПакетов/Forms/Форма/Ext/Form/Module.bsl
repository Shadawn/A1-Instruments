...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Пакет Из СписокПакетов Цикл
		...
		Строка.Зависимости = А1Э_Общее.ЗначениеСвойства(Пакет, "dependencies");
...
Процедура УстановитьОтмеченныеНаСервере(ВсеУстанавливаемые)
	...
	Для Каждого Имя Из ВсеУстанавливаемые Цикл
		...
		Строка.ВерсияТекущая = Расширение.ВерсияДоступная;
...
Функция ВерсияБольше(Версия1, Версия2, Схема)
	...
	Если Версия1Соответствует И Версия2Соответствует Тогда
		Если Схема = "SemVer" Тогда
			Семвер_ВерсияБольше(Версия1ПоСхеме, Версия2ПоСхеме);
		Иначе
...
Функция СемВер_Разобрать(Версия)
	...
	Семвер.Вставить("Основная", Число(Лев(Версия, ПерваяТочка - 1)));
	Семвер.Вставить("Малая", Число(Сред(Версия, ПерваяТочка + 1, ВтораяТочка - ПерваяТочка - 1)));
	Если НЕ А1Э_Строки_ТолькоЦифры(Семвер.Основная) Тогда Возврат Ложь; КонецЕсли;
	Если НЕ А1Э_Строки_ТолькоЦифры(Семвер.Малая) Тогда Возврат Ложь; КонецЕсли;
